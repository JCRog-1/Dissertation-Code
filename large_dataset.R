...
planar_dist_from_border_shape <- function(incorrectlabels){
  triangles <- planar.test$planar[incorrectlabels,]
  dist_iso_1 <- abs(triangles[,1]/triangles[,2] - 1)
  dist_iso_2 <- abs(1- 1/triangles[,1])
  border_distances <- cbind(dist_iso_1,dist_iso_2)
  closest_distance <- apply(border_distances,MARGIN = 1, FUN = function(x) min(x))
  return(closest_distance)
}
...
#Hopf Fibration----
center_tri <- function(X){
  c <- matrix(c(sum(X[,1])/3, sum(X[,2])/3),1,2)
  translation <- rbind(c,c,c)
  tri <- (X - translation)

  return(tri)
} #Translates a given triangle so that its centroid lies on the origin
M_extract <- function(X){
  M <- t(X)%*%t(defh(2))
  M_norm <- M/norm(M,type = 'F')
  return(M_norm)
} #Returns the unique M on the surface of S^3 correlating to the fibre GX
Hopf.conversion <- function(data_set){
  tris <- data_set$triangles
  
  centered_tris <- array_reshape(apply(tris,MARGIN = 3,FUN = function(x) center_tri(x)), 
                     dim = c(3,2,dim(tris)[3]),order = 'F')
  
  M <- array_reshape(apply(centered_tris,MARGIN = 3,FUN = function(x) M_extract(x)), 
                                 dim = c(2,2,dim(tris)[3]),order = 'F')
  
  set <- list(fibre = M, ao = data_set$ao, shape = data_set$shape)
  
  return(set)
}
Hopf_input <- Hopf.conversion(data_set = triangle_data_train)
Hopf.test <- Hopf.conversion(data_set = triangle_data_test)
#####
